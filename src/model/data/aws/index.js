...
async function readFragmentData(ownerId, id) {
  ...
  try {
    ...
    if (err.name === 'NoSuchKey') {
      logger.error({ err, Bucket, Key }, 'Fragment data not found in S3');
...
async function deleteFragment(ownerId, id) {
  // Create the DELETE params for S3
  const s3Params = {
    Bucket: process.env.AWS_S3_BUCKET_NAME,
    Key: `${ownerId}/${id}`,
  };

  // Create the DELETE command for S3
  ...
  // Create the DELETE command for DynamoDB
  ...
  try {
    ...
    logger.info({ ownerId, id }, 'Successfully deleted fragment data');
